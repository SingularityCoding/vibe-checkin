...
字段约束不在各个页面里各写一套。具体数值（内容 1–300 字、时长 5–600 且步长 5、标签最多 3 个每个 1–12 字、收获最多 140 字）统一定义在 `domain/constraints.ts`，Repository 层的防御性校验（`repositories/record-data.ts`、`repositories/local-preference/`）已经引用它强制执行。字符串清理、表单校验和错误文案仍由记录编辑 Feature 统一提供，应复用同一份常量而不是重新定义数值；Repository 的防御性校验是最后一道防线，不能持久化明显无效的数据，但不替代 Feature 层面向用户的校验体验。
...
P0 的 `repositories/record.ts` 是唯一组合入口。页面只从这里取得 `recordRepository`，不关心当前使用 Local 还是 Cloud 实现。`repositories/composition.ts` 已经提供对称的显式切换函数：`useLocalRepositories()`/`getLocalRepositories()` 和 `useCloudRepositories()`/`getCloudRepositories()`；`repositories/cloud-record/index.ts` 是 P0 预先搭好的 `CloudRecordRepository` 安全骨架，每个方法当前都明确 reject（不会静默成功或返回假数据），是 P1-08 的实现目标。P1-08 完成 Cloud 实现后，由老师在集成 PR 中调用 `useCloudRepositories()` 把组合入口切换到 Cloud；其他学生不修改这个入口。
...
Today 的 WXML 已提前放置 `<today-summary>` 和 `<today-activity>`。P1-01 与 P2-01 位于不同阶段，可以分别按自己的 Spec 修改 Today Page；两者不能修改对方独占的 Component / Feature。
...
日志页 WXML 已提前放置结构化筛选、关键词搜索和时间线三个组件。P1-05 和 P2-03 分别是各自阶段的日志 Page 主要集成人；P2-04 使用已接好的关键词组件入口，不需要与 P2-03 同时修改 Page。关键词组件发出的“清除全部筛选”事件由页面处理，同时重置日期、标签和关键词；P2-04 不直接操作 P2-03 的组件。
...
以上 5 个场景从 Today 页面启动，分别携带 `fixture=empty|today|history|read-error|reset`。此外还提供 6 个直接启动到其他页面的场景，全部使用 `history` Fixture，方便独立验收记录编辑、详情、日志、统计和设置：

- `测试场景 · 新建记录`（记录编辑，`mode=create`）
- `测试场景 · 历史编辑`（记录编辑，`mode=edit` 并携带已知记录 `id`）
- `测试场景 · 学习详情`（学习详情，携带已知记录 `id`）
- `测试场景 · 学习日志`
- `测试场景 · 学习统计`
- `测试场景 · 设置`

`App.onLaunch` 先根据 `fixture` 参数准备场景并写入 `fixtureReady`；六个页面（Today、学习日志、学习统计、学习详情、记录编辑、设置）都在首次读取 Repository 前通过 `fixtures/ready.ts` 的 `isFixtureReady()` 等待它完成，失败时展示各自页面已有的错误状态，不提前显示空数据。体验版和正式版会忽略所有 Fixture 启动参数。
...
每份正式 Spec 都会列出正常完成该用户旅程预计需要修改的文件，包括必要的 Page、Component、Feature、Repository 和测试文件。`pages/` 不作为全局不可修改目录。

同一个文件在同一 Phase 只分配给一项 Spec 作为主要修改范围。`app.ts`、`app.json`、`custom-tab-bar/`、公共领域类型、共享日期与导航、项目配置、环境 ID 和安全规则通常由 Starter 提供；只有某项用户功能确实要求改变它们时，才会在该 Spec 中明确写入。

如果实现过程中发现必须修改文件列表之外的公共契约，学生应在 PR 中说明缺失点、影响范围和理由。老师据此判断这是当前功能的必要改动，还是应先修正 Starter 的 integration fix；不能默默改变接口后要求同 Phase 其他同学跟随。

完整的 Phase 分配见 [Spec 分配矩阵](./specs/README.md)。
...
- 所有功能组件入口已在 Page 中接好；需要修改 Page 的任务已经在 Spec 分配矩阵中指定为该阶段唯一的页面主要集成人。

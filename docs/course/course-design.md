...
P0 预先注册全部路由，并由页面控制器负责加载数据、刷新和统一导航；学生主要实现各自拥有的组件与 feature module。
...
- 页面控制器只消费预先声明的组件事件；不要在同一 Phase 中让多个 PR 修改同一个页面控制器。
...
这一点是并行的关键：学生交付的是一个用户看得见、能操作的完整组件和其 feature logic，而不是一段辅助函数；但他们不会在课堂上互相抢改 `app.json`、Tab 配置、页面控制器或同一个 WXML 文件。

同一 Phase 的 Spec 不得直接导入另一项 Spec 独占目录里的实现。需要共享的类型、日期函数、Repository 和页面事件必须在 P0 中先冻结；需要组合多个功能的工作由 P0 页面控制器完成。
...
- 它只负责把关键词条件应用到传入的记录并输出结果摘要。页面控制器按冻结顺序组合结构化筛选与关键词筛选；P2-04 不直接导入或等待 P2-03 的实现。
...
| 风险 | 约束方式 |
| --- | --- |
| 两人修改同一个页面文件 | P0 页面控制器和组件槽位预先存在；每项 Spec 有唯一组件 / feature 目录。 |
...
7. **允许改动的文件与禁止触碰的文件**：让 PR 保持小而可 Review。
...
1. 按 [Starter Kit Contract](../starter-kit-contract.md) 实现 P0，并确认 16 个功能都已拥有无需修改 Page 的接入位置。
